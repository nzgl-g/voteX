...
export default function MonitoringPage() {
    ...
    const fetchData = async () => {
      try {
        ...
        if (sessionId === 'default' && userSessionsData.length > 0) {
          router.replace(`/team-leader/monitoring/${userSessionsData[0]._id}`)
          return
        }
  ...
  if (loading) {
    return (
      <>
        <SiteHeader title="Loading monitoring dashboard..." />
        ...
        <div className="min-h-screen bg-background flex items-center justify-center">
          <div className="bg-white dark:bg-slate-800 p-8 rounded-xl shadow-sm max-w-md">
            ...
            {userSessions.length > 0 ? (
              <button 
                onClick={() => router.push(`/team-leader/monitoring/${userSessions[0]._id}`)}
