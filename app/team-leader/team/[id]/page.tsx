...
export default function TeamPage() {
    ...
    const fetchData = async () => {
      try {
        ...
        if (sessionId === 'default' && userSessionsData.length > 0) {
          router.replace(`/team-leader/team/${userSessionsData[0]._id}`)
          return
        }
  ...
  if (loading) {
    return (
      <>
        <SiteHeader title="Loading team management..." />
        ...
        <div className="min-h-screen bg-background flex items-center justify-center">
          <div className="bg-white dark:bg-slate-800 p-8 rounded-xl shadow-sm max-w-md">
            ...
            {userSessions.length > 0 ? (
              <button 
                onClick={() => router.push(`/team-leader/team/${userSessions[0]._id}`)}
  ...
  return (
    <>
      ...
      <main className="min-h-screen bg-gradient-to-b from-background to-muted/30">
        <TeamManagementDashboard sessionId={sessionId} />
      </main>
    </>
  )
}
