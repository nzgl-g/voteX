...
export default function Step3Lifecycle({ formData, updateFormData }: Step3Props) {
  ...
  return (
    <div className="animate-in fade-in duration-500">
      ...
      {formData.voteType === "election" && (
        <div className="mb-8">
          <div className="flex items-center p-4 rounded-xl bg-primary/5 border border-primary/20 mb-4">
            <Switch
              id="nomination-phase"
              checked={formData.hasNomination}
              onCheckedChange={(checked) => updateFormData({ hasNomination: checked })}
              className="data-[state=checked]:bg-primary"
            />
            <Label htmlFor="nomination-phase" className="font-medium text-foreground ml-3 cursor-pointer">
              Enable Nomination Phase
            </Label>
            <div className="ml-auto text-xs bg-primary/10 text-primary px-2 py-0.5 rounded-full">
              {formData.voteType === "election" ? "Elections Only" : ""}
            </div>
          </div>
          
          {formData.hasNomination && (
            <div className="relative ml-4 sm:ml-6 pl-4 sm:pl-6 before:absolute before:left-0 before:top-0 before:bottom-5 before:w-0.5 before:bg-primary/30">
              <div className="mb-8 relative">
                <div className="absolute -left-8 sm:-left-10 top-0 size-7 sm:size-8 rounded-full bg-primary/10 text-primary flex items-center justify-center text-sm font-medium">1</div>
      ...
      <div className="relative ml-4 sm:ml-6 pl-4 sm:pl-6 before:absolute before:left-0 before:top-0 before:bottom-0 before:w-0.5 before:bg-primary/30">
        <div className="mb-8 relative">
          <div className="absolute -left-8 sm:-left-10 top-0 size-7 sm:size-8 rounded-full bg-primary/10 text-primary flex items-center justify-center text-sm font-medium">{formData.voteType === "election" && formData.hasNomination ? "2" : "1"}</div>
          <div className="bg-card rounded-xl shadow-sm border border-border p-3 sm:p-5">
            <h3 className="font-medium text-foreground mb-4 flex flex-wrap items-center gap-2">
